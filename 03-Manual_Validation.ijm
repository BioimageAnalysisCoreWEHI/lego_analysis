#@ File (label="Choose folder with extracted mets (raw)",style="directory") MET_PATH_tmp
#@ File (label="Choose folder with masks",style="directory") MASK_PATH_tmp
#@ File (label="Table with scoring output",style="file") tablePath
...
#@ int (label="Min Intensity",style="Slider",min=0,max=32000) minInt
#@ int (label="Max Intensity",style="Slider",min=0,max=32000) maxInt
#@ Boolean (label="Show MIP of data and mask?",value=false) SHOW_MIP
#@ String (label="Show:",choices={"With background","Without background","Both"}, style="radioButtonVertical") what_to_show
...
MASK_PATH = MASK_PATH_tmp + File.separator();


var dir2 = MASK_PATH + "CORRECTED_METS";
...
function displayMet(MET_NUMBER){
	...
	data_with_bg_path = MET_PATH + "MET_"+MET_NUMBER+"_with_bg.tif";
	maskPath = MASK_PATH + "MET_"+MET_NUMBER+".tif.jpg";
	maskPath3D = MASK_PATH + "MET_"+MET_NUMBER+".tif_stackMask.tif";
	...
	getLocationAndSize(x, y, width, height);
		...
		open(maskPath);
		if(placeWindows){setLocation(x+width,y);}
	}
	getLocationAndSize(x1, y1, width1, height1);

	if ((what_to_show == "Without background") || (what_to_show == "Both")){		
		open(dataPath);
	...
	if ((what_to_show == "With background") || (what_to_show == "Both")){		
		open(data_with_bg_path);
		Stack.setChannel(1);run("Cyan");
		Stack.setChannel(2);run("Yellow");
		Stack.setChannel(3);run("Magenta");
		Stack.setActiveChannels("1110");
	...
	//print(x+width, y1+height1);
	
	
	if(placeWindows){
		if(SHOW_MIP){
			setLocation(x+width,y1+height1);
		}else{
			setLocation(x+width,y1);
		}
	}
	getLocationAndSize(x1, y1, width1, height1);
	Stack.setActiveChannels("1110");
	...
	open(maskPath3D);
	if(placeWindows){setLocation(x1+width1,y1);}
...
function displayMet2(MET_NUMBER){
		
	dataPath = MET_PATH + "MET_"+MET_NUMBER+".tif";
	maskPath = MET_PATH + "masks2"+File.separator+"MET_"+MET_NUMBER+".tif.jpg";
	maskPath3D = MET_PATH + "masks2"+File.separator+"MET_"+MET_NUMBER+".tif_stackMask.tif";
	//tablePath = MET_PATH + "masks2" + File.separator() + "Table name.csv";
	...
	makeBars(a);
	getLocationAndSize(x, y, width, height);
	if(placeWindows){setLocation(x + 1000,y);} 
	
	
	open(maskPath);
	if(placeWindows){setLocation(x+width + 1000,y);} 
	getLocationAndSize(x1, y1, width1, height1);
	
	open(dataPath);
	print(x+width, y1+height1);
	
	if(placeWindows){setLocation(x+width+1000,y1+height1);}
	getLocationAndSize(x1, y1, width1, height1);
	Stack.setActiveChannels("1110");
	getDimensions(width, height, channels, slices, frames);
	Stack.setSlice(slices/2);
