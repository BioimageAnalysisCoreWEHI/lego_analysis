#! /bin/bash
#SBATCH --job-name=czi2zarr #Name to give the job
#SBATCH --time=3:30:00 #Time to allow job to run for
#SBATCH --ntasks=1
#SBATCH --output=/vast/scratch/users/microscopy/outputs/czi_to_zarr-%A_%a.out #Where to put output logs

#SBATCH --cpus-per-task=24 #Only use more than one if code is already multithreaded
...
#run the analysis
python czi2zarr.py --input "$1" --output $2
